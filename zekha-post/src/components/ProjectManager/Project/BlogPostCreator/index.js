...
const BlogPostCreator = ({curProj, uid}) => {
 ...
 const [ submitDisabled, setSubmitDisabled ] = useState(true);
  ...
  const data = [
    {
      title: 'Listicle',
      template: <Listicle submitButton={ [ submitDisabled, setSubmitDisabled ] } post={[finalPost, setFinalPost]}  curTemp={[template, setTemplate]} curProj={[currentProject, setCurrentProject]} uid={uid} />,
  ...
  return (
    ...
    dataSource={data}
    renderItem={(item) => (
      <TemplateCard submitButton={ [ submitDisabled, setSubmitDisabled ] } curTemp={[template, setTemplate]} post={[finalPost, setFinalPost]} curProj={[currentProject, setCurrentProject]} uid={uid} item={item} content={item.content} />
    )}
  />
        </Col>
    </Row>
